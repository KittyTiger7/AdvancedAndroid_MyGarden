...
public class PlantWidgetProvider extends AppWidgetProvider {
    ...
    static void updateAppWidget(Context context, AppWidgetManager appWidgetManager,
        ...
        int widgetWidth = bundle.getInt(AppWidgetManager.OPTION_APPWIDGET_MIN_WIDTH);

        // Instruct the widget manager to update the widget, based on widget's min width
        if (widgetWidth < 300) {
            Toast.makeText(context, "widget width less than 300", Toast.LENGTH_SHORT).show();
            appWidgetManager.updateAppWidget(appWidgetId, getSinglePlantRemoteView(context, imgRes, plantId, showWater));
        }
        else appWidgetManager.updateAppWidget(appWidgetId, getGardenGridRemoteView(context));
    ...
    private static RemoteViews getGardenGridRemoteView(Context context) {
        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.widget_grid_view_layout);
